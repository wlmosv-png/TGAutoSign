...
**架构更新**：账号隔离从「约定」变成「结构」 · 核心文件拆出 6 个职责单一的类 · 纯逻辑可单测 · 存储键名收敛到唯一真相源 · 落盘策略显式化

**修复**：账号串号：六条路径全部改为锁定账号（重要） · 越界保护反而读到别人的分区（重要） · 失败目标被反复重签（重要） · 同一账号重复排期发送（重要） · 明明签到成功，却被判失败并退回「退避中」（重要） · 「等面板」永远超时走兜底（重要） · 同类问题全量清查：还有三处会把成功当失败 · 「待确认」池按账号隔离，并自动迁移旧数据 · 「待确认」是个死状态：看得到、点不动（重要） · 回复判定用错账号（重要） · 全账号签到时结果串到别的账号 · 清空配置会残留状态，导致重新添加的 bot 状态复活（重要） · 清空配置会误删账号级待确认池 · 按钮反复过期导致无限重试 · 不回结果的机器人每天白等超时 · 设置界面保存时闪退 · 打开「运行日志」会卡顿 · 启动日志只落盘第一行 · 冷启动 30 秒内点 bot 按钮毫无反应 · 跨天窗口下补签时段变成全天 · 跨端同步的配置在本机不生效 · 连点多个 bot 按钮时只有第一个能被学到 · 日志里的链路编号会跨账号重复 · 热重载后两个实例并行跑 · 启动日志里的数字标题渲染成错误字形 · 花体字有豆腐块 · 「待确认」的「重试」按钮用错账号

**新增**：日志与诊断包显示详细版本信息 · 宿主友好名支持英文

**工具**：仓库里的 `build.sh` 缺三道门禁 · 纯逻辑单测从 115 条扩到 174 条 · 接线自检的成员清单里有早已删除的方法

### v1.5.8 (121) — 2026-09-24

**修复**：子面板弹出来变成亮色，已修 · 主题日志改为「每次变化都记」 · 「未识别 bot 回复」不再刷红 · 同一次点击不再被重复处理 · i18n 门禁补上白名单漏洞，并修掉 18 处历史漏译 · 门禁能抓住「用了封装方法但忘了进字典」 · 后台心跳按需降频，夜里不再每 45 秒醒一次 · 定时任务不再重复排队 / 丢任务 · 面板刷新触发不再与心跳重复发送 · 机器人名字只显示一次数字 ID，之后再也取不到 · 「今天已签到」这类回复现在认得出了 · 「本条不是签到结果」不再让人误以为失败 · 「自动判定」被关掉时不再静默 · 新装用户点 bot 按钮学不会目标（重要） · 捕获模式在 Nagram \/ 官方版上无效 · 拒绝原因被谎报 · 界面显示的账号号错乱（如显示「账号10」） · 日志账号前缀过期 · 跨午夜签到窗口现在真的能用 · 跨客户端同步：账号不再串位 · 收到其他客户端的配置后会重排时刻表 · 发送中状态的有效窗口与失败撤销对齐（90 秒 → 30 分钟） · 跨端同步节流 · 发版前自动检查测试标记

**新增**：判定词改成开关式 · 新增「宽松模式（来者不拒）」

**可靠性**：补上纯逻辑单测，并挂进构建门禁 · bot 回复认不出来时不再静默 · 关键路径的异常不再被静默吞掉

**诊断**：诊断包新增学习与 hook 状态 · 诊断包头部显示补丁标记 · 诊断包新增账号字段原始值
