...
[更新日志](CHANGELOG.md) · [模块发布页（Xposed-Modules-Repo）](https://github.com/Xposed-Modules-Repo/io.github.wlmosv_png.tgautosign/releases) · 当前版本 **v1.4.0 (108)** · 作者 wlmosv
...
**签到核心**
- 自动学习签到目标（默认关闭）：打开后点按 bot 按钮即记录，可限定仅命中关键词才加，防误加；回调按钮推荐用「捕获 / 调试台」手动绑定，更可控
- 每天每号只签一次；已签提示；断网后自动补签；打开聊天 / 网络恢复自动触发
- 发送层自动识别签到指令（仅 bot 会话、含关键词），不碰普通聊天
- Bot 回复语义判定 + 错误分类：永久放弃 / 限流冷却 60s / 指数退避 5m→15m→45m→2h→4h

**签到可靠性与多账号（v1.4.2 重点）**
- 同一天的退避与重试上限按 `retry_day_` 判定：失败的目标不再被反复重发刷 bot，换天自动重新开一轮
- 目标缓存、判重、「今天已签」写入始终跟随当前账号，不再用上一个账号的列表判重（那会导致学不进去、覆盖错位）
- 取不到 bot 会话数据时先从会话列表兜底解析 peer；仍不行才计入退避与上限，并说明下一步怎么做；目标是群或频道则直接判不可签
- 「签全部账号」逐账号汇报（目标数 / 已签 / 待重试），没有目标的账号会直说，不再报一句「已执行」了事
- 「复制目标到其它账号」：已有目标一键同步给其它账号，只复制目标本身，不带「今天已签」与失败记录
...
**回调按钮签到（v1.4.0 重点）**
- ➕「🔘 回调按钮(捕获)」：去 bot 会话点一下它的按钮 → 列出该消息所有内联按钮 → 点选绑定，可连点绑多个；同按钮自动去重，绑定不受关键词限制
- 🧪 测试 / 🔬 调试台：列出当前面板全部按钮，点任意一个即时发一次回调并把机器人返回显示出来，最适合排查「哪个按钮 / data 才对」
- 每条目标可自带「前置命令序列」（如 `/start`），签到 / 测试前先依次发送把面板拉出来再重放，解决「bot 不主动推面板就签不上」
- 🔁 重绑为回调：把 1.3.1 期间被误判成「指令」的旧回调按真实按钮重建
- 全新卡片界面、深浅色自适应、运行日志分级、使用教程、自诊断；配置删除跨全部账号彻底清理，重开 TG 不再复活

**多账号**
- 学习目标、当天已签、重试与退避全部按当前账号隔离存放，互不覆盖
- 每次入口即同步当前账号并重载目标（不再依赖 30 分钟轮询）；仍只签当前选中账号，不替后台账号发消息
- **🌐 一键签全部账号**：`/jmb` 菜单直接对所有已激活账号分别签到（v1.3.0 新增）
- 官方版与第三方客户端偏好文件独立，可分别配置

**管理与维护（全部在 `/jmb` 菜单内完成）**
- 📋 目标列表（查看 · 测试 · 编辑 · 删除） / ➕ 添加（文本 / 捕获回调） / 🔬 回调调试台 / 🚀 立即签到 / 🌐 签全部账号
- 🧹 清空所有配置（跨全部账号彻底清） / 📖 使用教程 / 🩺 自诊断
- 📄 运行日志（错误/警告/成功/普通/调试五级，支持搜索、按级别筛选、正序倒序切换、长按单行复制、一键清空；日志同时落盘，重启后仍可翻查历史，上限约 1.3MB 自动轮转） / 🧾 导出运行日志
- ⚙️ 设置：签到关键词、每日重试上限、自动学习 / 网络层自动学习 / 关键词过滤开关、全局唤醒命令
- 🔄 检查更新：启动静默查（12 小时冷却）+ 手动强制查，下载到系统「下载」目录后交系统安装器确认（不申请安装权限、不自动安装）
- 📤 导出配置 / 📥 导入配置：目标、关键词、重试上限、当天状态存成 json；导入会先列出候选备份（文件名、版本、导出时间、项数），再选「合并」或「覆盖」，结束时报当前账号目标数的前后对比；不认识的键一律不写入
...
- 签到依赖 bot 的消息文本与按钮：bot 改了回复文案可能被误判为失败（v1.4.2 已收紧判失败词表，仍建议观察一天）
- fork 的兼容性核对停在 v1.4.0（官方 12.10.1 / 官网 web / Nagram XF 静态一致，Nagram XF 曾真机实测）；本版改动不涉及反射点
...
| 版本 | versionCode | 主题 |
| --- | --- | --- |
| v1.4.2 | 111 | 签到调度与多账号修正 + 运行日志重做 + 跨账号复制目标 + 导入流程重做 |
| v1.4.0 | 108 | 回调捕获/绑定/调试台 + 每条目前置命令 + 卡片界面 + 教程与自诊断 |
| v1.3.1 | 107 | 修复回调按钮签到在 TG 12.10.1 不可用（按钮 data 读取 + 补 msg_id） |
