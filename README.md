...
**定时签到（v1.5.4）**
- 开启后只在签到窗口内签，窗口外所有自动触发一律不动作；时间按钮选择、错开间隔可配（0=自动均分）
- 当日时刻表按目标均分时段随机取时、互不重叠；今日计划卡片显示已签/待签时刻

**界面体验（v1.5.4）**
- 自绘终端卡片对话框（告别系统框）；设置页分组美化；日志页按目标过滤 + 一键诊断包 + 配色重做
- 主菜单分 5 组；目标列表三列对齐 + 状态色 + 排序；标题动效轮换
...
### v1.5.4 (117) —— 定时签到体系 + 日志升级 + 界面美化

- ⏰ **定时签到**：开启后只在窗口内签，窗口外零动作；签到时间点按钮选择，错开间隔可配置
- 📅 **今日计划**：每目标卡片显示已签实际时刻 / 待签计划时刻
- 📄 **日志升级**：按目标过滤、一键诊断包、加载更多、配色重做（亮暗双套）
- 🖌 **界面**：自绘终端卡片对话框（告别系统框）；设置页分组美化；状态卡下次签到倒计时
- 🐛 修复：启动/面板补签遵守窗口；多目标随机间隔发送
- 🔌 主题判定分宿主适配（官方/Nagram 12.10.3 / ExteraLess）
