...
## v1.4.1 (110)
修复审计发现的问题（行为不变的部分没动）：
- 重试上限与退避在同一天内失效：跨天重置改成按 retry_day_ 判定，失败不会再变成无限重发。
- 「签全部账号」被自己的 60 秒节流挡住：整轮走 force，第 2..N 个账号不再空跑。
- 打开任意聊天不再弹「今天已经签到过了」，每天最多提示一次；启动/注入不再弹两条 Toast（改看 自诊断/日志）。
- 账号切换即时跟随：目标缓存、学习、判重不再用上一个账号的列表；集合与日期格式改为线程安全。
- 编辑目标清空「前置命令」现在会真的落盘；导入的旧备份若类型不一致不再让目标静默消失。
- /jmb 只在恰好等于 /jmb 或 "/jmb 参数" 时拦截，不再吞掉 /jmbx；发送中状态 90 秒自动过期，卡死可自愈。
- 捕获模式 2 分钟未点按钮自动解除；Activity 销毁即释放，弹窗前判活。
- 网络层自动学习加开关（设置里可关，主界面显示状态）；回复判失败的关键词收紧，减少误撤销。
- 导入配置先显示文件内容与合并语义再确认；下载安装包校验 sha256sum.txt；检查更新失败不再占用 12 小时冷却；限流报错说人话。
- 清空配置的保护名单修正（不再顺手抹掉教程标记）；jlog 时间戳格式修正。
- 僵尸目标处理：取不到会话数据也计入退避与上限；目标是群/频道则当天放弃并说明原因。
- 全账号签到逐账号汇报；新增「复制目标到其它账号」（只复制目标，不带已签与重试状态）。
- 运行日志重做：分级别（错误/警告/成功/普通/调试）、落盘 800 条历史、搜索、筛选、顺序切换、长按复制、清空、导出带级别。
- Toast 策略：启动汇总每天一次；签到结果 8 秒内合并成一条；同类提示 5 分钟去重。
- 导入配置改为「选备份 → 选合并或覆盖 → 显示目标数前后对比」，不认识的键不写入。
- 历史无前缀老键一次性搬正；删除与清空统一按"是否目标键"判定，设置项一律保留。
- 自诊断逐账号列出目标数，内部术语改人话（不再露出 0 基账号号）。
