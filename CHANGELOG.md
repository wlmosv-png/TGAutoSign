...
## v1.3.0 (versionCode 106) —— 大功能版：回调按钮签到 + 多指令 + 全账号签到

- **新增：回调按钮（inline button）签到支持。** 现在在聊天里点一次 bot 的回调型签到按钮，模块会自动学习（记录按钮文案 + 回调 payload data + hash），此后每天自动重放 `getBotCallbackAnswer` 完成签到。文本指令与回调按钮可并存。
  - 学习通道：按钮点击 hook（带按钮文案，推荐）+ 网络层兜底（按钮 hook 未捕获时按 data 内容学习）
  - 自动排除：链接/游戏按钮（无回调 data）不学习
  - 标记/失败/退避/限流逻辑与文本指令完全一致
- **新增：一个 bot 可登记多条签到目标（多指令）。** 目标从"一 bot 一指令"升级为"一 bot 多条目"，文本与回调按钮都可多份；列表按条目展示与删除，互不影响。
  - 旧数据零迁移：v1.2.x 的 `acc{N}_learned_<did>` 原键直接读取，键名不变
- **新增：🌐 签全部账号。** `/jmb` 菜单一键对当前客户端内所有已激活账号分别执行签到（每账号独立目标集、独立发送通道），不切换界面账号。
- **变更：目标列表显示条目类型标记**（🔘 = 回调按钮），状态/上次签到按条目独立统计。
- **兼容：** 签到数据、导出 json 格式、更新通道与签名 key 均不变；覆盖安装即可，旧目标原样保留。
